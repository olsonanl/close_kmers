/*
 * Build a NuDB database of new signature kmers.
 *
 */
...
int main(int argc, char *argv[])
{
    ...
    std::cerr << "Default concurrency: " << tbb::info::default_concurrency() << std::endl;
    tbb::global_control global_limit(tbb::global_control::max_allowed_parallelism, n_threads);
    // tbb::global_control(tbb::global_control::max_allowed_parallelism, n_threads);
    std::cerr << "Default concurrency after setting to " << n_threads << ": " << tbb::info::default_concurrency() <<
	" " << std::endl;
